...
def test_every_name_in_the_list_is_refused_as_a_header_point():
    """DERIVED FROM THE LIST, not typed beside it. The parametrised case
    above spells four literals and would go on passing if a fifth name
    reached `CREDENTIAL_HEADERS` with no refusal behind it; this iterates
    the list itself, so a name the extension refuses and `unprobeable` does
    not cannot be added silently. It is what is left of fix round 5's
    `test_the_two_questions_read_the_same_list`, whose second question
    (`credentials_carried`) went with the retirement rule it served."""
