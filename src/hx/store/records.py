...
def upsert_finding(conn: sqlite3.Connection, *, engagement_id: str, candidate,
                   ...
                   host: str | None = None,
                   issue_type_id: str | None = None) -> str:
    ...
    `surface_id`, `host` AND `issue_type_id` ARE KEYWORD-ONLY AND DEFAULT TO
    ...
    `issue_type_id` exists for a NARROWER question a retest also needs
    answered: not just which surface, but which CHECK found it. F1 of the
    task-6 review, HIGH: keying a retest on surface alone means a finding
    reads as `observed=0` (which a report renders as "fixed") when the surface
    was visited but the check that owns the finding raised, went
    inconclusive, was skipped by the budget, or was not even in this run's
    `checks` -- none of which is "looked and it's gone". `hx.scan._write_finding`
    already has `check.id` in hand (it is the first field of `dedupe_key`,
    S5's canonical string), and this column -- declared, unused, `NULL`
    everywhere until now -- is where that identity belongs rather than a
    second place parsing it back out of `dedupe_key`'s own string format.
    `hx.scan._mark_unobserved` reads it back to require an exact
    ...
    conn.execute(
        ...
        " host, issue_type_id, first_seen_run, last_seen_run)"
        " VALUES(?,?,?,?,?,?,?,?,?,?, 'check', 'new', ?,?,?,?,?,?,?,?,?)"
